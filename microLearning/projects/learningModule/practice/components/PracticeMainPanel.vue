  ...
  <div class="practice-main-panel">
    <div class="practice-bg-container">
      <div class="flex-button-container">
        <practice-main-panel-item :buttonText="'模拟考试'" :iconSrc="mockExamSrc" @on-click-item="beginSimulationExamine" :isBigIcon="true"></practice-main-panel-item>
      </div>
    ...
    <div class="practice-function-container">
      <div v-for="(buttonRow, index) in practiceButtonList" :key="index" class="flex-button-container">
        <div v-for="item in buttonRow" :key="item.value" class="flex-button" :class="{'no-content-flex-button':!item.title}" @click="practiceButtonClick(item)">
...
<script>
import { practiceButtonTypes } from '../constants/examConst';
import PracticeMainPanelItem from './PracticeMainPanelItem';

export default {
  name: 'practice-main-panel',
  components: { PracticeMainPanelItem },
  data() {
    return {
      ...
      practiceButtonList: [
        [
          { value: practiceButtonTypes.ORDER, title: '顺序练习', iconSrc: require('@/assets/img/examination/icon_Sequentialexercise.png') },
          { value: practiceButtonTypes.TYPE, title: '题型练习', iconSrc: require('@/assets/img/examination/icon_typeexercises.png') }
        ],
        [
          { value: practiceButtonTypes.RANDOM, title: '随机练习', iconSrc: require('@/assets/img/examination/icon_random.png') },
          { value: practiceButtonTypes.MY_COLLECTION, title: '我的收藏', iconSrc: require('@/assets/img/examination/icon_collection.png') }
        ],
        [{ value: practiceButtonTypes.MY_ERROR, title: '我的错题', iconSrc: require('@/assets/img/examination/icon_wrong.png') }, { value: practiceButtonTypes.SPACE }]
      ]
    };
  },
  props: {
  ...
  methods: {
    ...
    toSimulationExamineRecordPage() {
      this.$router.push({ name: 'simulationExamRecord' });
    },
    // 前往排行榜页面
    toRankingListPage() {
    ...
    toPracticeDetailPage(trainingType, reBegin) {
      ...
      this.$router.push({ name: 'practiceDetail', params: param });
    },
    // 前往题型练习页面
    toTypePracticePage() {
    ...
    queryExistSimulationExamineByCategoryID(successCallback) {
      ...
      this.$http.get('/training/getTrainingExamAppByCategoryId', { params: param }).then(
        res => {
    ...
    queryExistRankingInfo(successCallback) {
      ...
      this.$http.get('/training/checkSomeOneOnRankingByCategoryId', { params: param }).then(
        res => {
          if (res && res.data && successCallback) {
            successCallback(res.data);
          }
    ...
    queryOrderPracticeInfo(successCallback) {
      ...
      this.$http.get('/training/getTrainingDoneTopicCount', { params: param }).then(
        res => {
          if (res && res.data && successCallback) {
            successCallback(res.data);
          }
    ...
    queryErrorOrFavoritePracticeInfo(option) {
      ...
      this.$http.get('/training/getEorrorOrFavoritePracticeHasTopic', { params: param }).then(
        res => {
          if (res && res.data) {
